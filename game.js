...
function makeCreature(speciesId, level) {
  ...
  const maxHp = s.baseHP + level * 3;
  return { speciesId, level, xp:0, maxHp, hp:maxHp, atk:s.baseAtk + level * 2, status:null };
}
...
function tryMove(dx, dy) {
  ...
  if (enc && state.teamIdx.length && Math.random() < enc.chance) {
    ...
    startWildBattle(makeCreature(sid, lvl));
  }
}
...
function startWildBattle(enemy) {
  ...
  openBattle(`A wild ${spec(enemy).name} (Lv ${enemy.level}) appeared!`);
}

function startTrainerBattle(trainer) {
  const a = firstAliveIdx();
  if (a === undefined) { hudMsg("Heal your team at the ⛺ tent before battling!"); return; }
  const queue = trainer.team.map(([sid,lvl]) => makeCreature(sid,lvl));
...
function cardHTML(c, label) {
  ...
  return `<div class="nm">${spec(c).icon} ${spec(c).name} <span class="typeTag">${spec(c).type}</span>${st}</div>
...
function spriteHTML(c) {
  return `<span class="spriteEmoji">${spec(c).icon}</span>
    <div class="platform"></div>
    <div class="spriteName">${spec(c).name} Lv${c.level}</div>`;
}
...
function openTeam() {
  ...
  state.collection.forEach((c, i) => {
    ...
    row.innerHTML = `<span style="font-size:22px">${spec(c).icon}</span>
      <b>${spec(c).name}</b> <span class="typeTag">${spec(c).type}</span>
